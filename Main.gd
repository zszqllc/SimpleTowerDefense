...
func _ready():
	font = load("res://Resource/Aa厚底黑.ttf")
	logo_tex = load("res://Resource/icon.svg")
	menu_tex = load("res://Resource/back.png")
	
	UserStorge.set_max_waves(0)
...
func _load_game():
	get_tree().change_scene_to_file("res://levels/LevelChange.tscn")
pass
...
func save():
	...
	var UnitFile = FileAccess.open("user://Unit.json", FileAccess.WRITE)
	UnitFile.store_string(unit)
	UnitFile.close()
	
	var EnemyFile = FileAccess.open("user://Enemy.json", FileAccess.WRITE)
	EnemyFile.store_string(enemy)
	EnemyFile.close()
	
	var LevelFile = FileAccess.open("user://Level1.json", FileAccess.WRITE)
	LevelFile.store_string(Level)
	LevelFile.close()
